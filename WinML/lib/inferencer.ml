...
let infer f =
  infer_func Context.empty Subst.empty f |> TI.run 0

let%test "f x = 1 + 1" =
  let f = 
    Function ("f", ["x"], BinOp(Add, Const(CInt 1), Const(CInt 1)))
  in
  let t, _ =
    match infer f with
    | Monad.Result.Ok x -> x
...
let%test "f x = if x then 1 else x" =
  let f = 
    Function ("f", ["x"], IfThenElse(Var "x", Const (CInt 1), Var "x"))
  in
  match infer f with
...
let%test "factorial" =
  let fac = 
    RecFunction ("f", ["n"], 
      IfThenElse(BinOp(Eq, Var "n", Const(CInt 1)),
        Const(CInt 1),
        BinOp(Mul, Var "n", Application(Var "f", BinOp(Sub, Var "n", Const(CInt 1)), []))
      ))
  in
