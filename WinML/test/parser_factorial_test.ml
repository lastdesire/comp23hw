...
let print_fac_test_expr = function 
  | Winml.Parsed_tree.IfThenElse(BinOp(Eq, Var "n", Const(CInt 1)),
        Const(CInt 1),
        BinOp(Mul, Var "n", Application(Var "fac", BinOp(Sub, Var "n", Const(CInt 1)), []))
      ) -> Printf.printf "IfThenElse(BinOp(Eq, Var 'n', Const(CInt 1)), Const(CInt 1), BinOp(Mul, Var 'n', Application(Var 'fac', BinOp(Sub, Var 'n', Const(CInt 1)), []))"
